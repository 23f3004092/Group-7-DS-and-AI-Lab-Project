...
* **RAG/NLP corpus (this section's contribution):** `pdf_inventory_clean.csv` (cleaned document inventory, 170 documents), extracted `.txt` files per PDF, `excluded_unreadable_docs.csv`, `excluded_near_duplicate_docs.csv`, `02_pdfs_rag_eda.ipynb` (EDA notebook), `corpus_overview.png`, `word_frequency.png`, `PDF_Chunking.ipynb` (chunking notebook), `pdf_chunks.csv` / `pdf_chunks.jsonl` (1,451 chunked records ready for Milestone 3 embedding).
