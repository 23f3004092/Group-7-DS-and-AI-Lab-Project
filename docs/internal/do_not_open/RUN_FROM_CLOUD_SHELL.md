...
4. **Hugging Face** — accept the Gemma licence at <https://huggingface.co/google/gemma-3-4b-it> and make a token at <https://huggingface.co/settings/tokens>.
...
Open a Colab notebook and paste the cells from `upload_artifacts_from_colab.py` **in order**:
1. **Cell 1** — Sign into GCP, Drive, and Kaggle.
2. **Cell 2** — Set `PROJECT`, `BUCKET`, `HF_TOKEN` (match your `.env` from Step 3).
3. **Cell 3** — Upload RAG snapshot from Drive → `$BUCKET/qdrant/` (pulls manifest.json + snapshot, as-is).
4. **Cell 4 (CRITICAL — requires GPU)** — **Merges your LoRA adapter directly into base Gemma-3-4B**, then saves the merged model to `$BUCKET/generator/merged/`. This is **essential for fast inference at serving time** — without the merge, the gateway loads base + adapter separately at runtime, causing ~2× slower inference due to per-step adapter overhead. The merge happens **once** in Colab; on the VM it just loads the merged weights. ✅ **You can switch back to CPU runtime after cell 4 finishes.**
...
5. **Cell 5** — Download + upload IEG + Vision models from Kaggle → `$BUCKET/ieg/` and `$BUCKET/vision/`.
...
- If `generator/merged/` is missing: the gateway will still work but fall back to loading base + adapter at startup (slower inference). Rerun cell 4.
...
4. **Loads the merged Gemma model** (from `generator/merged/` if it exists, else base + adapter as fallback).
...
| **Adapter loaded at runtime** | Skip cell 4; gateway loads base + adapter on startup | $0 extra | **~2× slower** due to per-step adapter overhead |
...
# Paste cells 1-5 from upload_artifacts_from_colab.py
