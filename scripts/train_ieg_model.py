"""
train_ieg_model.py — IEG Training Script v2

Multi-task transformer (l3cube-pune/hing-mbert-mixed) with intent, NER, and guardrail heads.
Features: AdamW + linear warmup/decay, class-weighted guardrail loss, label smoothing,
early stopping, EDA, span-level NER F1, eval_summary.json output.
See outputs/ieg_model/USAGE.md for full model I/O specification.
"""
...
DATA_PATH = ROOT / "data" / "processed" / "kcc" / "kcc_train_99.csv"
...
MAX_LEN         = 64
BATCH_SIZE      = 32
EPOCHS          = 5         # early stopping will likely fire before this
LR              = 3e-5      # lower than v1 (5e-5); more stable with scheduler
WEIGHT_DECAY    = 0.01
...
NON_AGRI_CSV = ROOT / "data" / "processed" / "kcc" / "final_non_agri_data01.csv"
...
def _build_guardrail_df() -> pd.DataFrame:
    ...
    return pd.DataFrame([{
        "QueryText":       text,
        "intent_label":    "general",   # masked by _is_real_kcc=0; value is irrelevant
        "guardrail_label": 1,
        "_char_spans":     "[]",
        "_is_real_kcc":    0,
...
def load_data(tokenizer):
    ...
    if ADD_AG_NEWS_NEGATIVES:
        from datasets import load_dataset
        print("Loading AG News off-domain negatives...")
        ag    = load_dataset("fancyzhx/ag_news", split="train")
        n     = min(len(df) // 4, 5000)
        texts = ag.shuffle(seed=SEED).select(range(n))["text"]
        ag_df = pd.DataFrame({
            "QueryText":       texts,
            "intent_label":    "general",  # masked; _is_real_kcc=0
            "guardrail_label": 1,          # treated as dangerous for extra coverage
            "_char_spans":     "[]",
            "_is_real_kcc":    0,
        })
        all_dfs.append(ag_df)
        print(f"Added {len(ag_df)} AG News rows")
    ...
    # ── 80 / 10 / 10 split — stratified by guardrail_label ──────────────────
    train_df, temp_df = train_test_split(
        full_df, test_size=0.20, random_state=SEED,
        stratify=full_df["guardrail_label"],
    )
...
def train():
    ...
    eval_summary = {
        ...
        "config": {
            ...
            "patience":          PATIENCE,
            "ner_labels":        NER_LABELS,
            "augment_ag_news":   ADD_AG_NEWS_NEGATIVES,
        },
    }
